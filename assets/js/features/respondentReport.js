...
import { mapAnswer, normalizeQuestion, setStatus } from "../utils.js";
...
export async function loadRespondentAnswers() {
  ...
  Object.keys(series).forEach((q) => {
    const f = series[q][earliest];
    const t = series[q][latest];
...
function renderExtremes(rows, series, periods, map) {
  ...
  const prevS = document.getElementById("extremes-section");
  const prevH = document.getElementById("extremes-section-header");
  if (prevS) prevS.remove();
  if (prevH) prevH.remove();
